...
{
   BGR8(1, 3, opencv_imgproc.COLOR_BGR2RGBA, opencv_imgproc.COLOR_RGBA2BGR, avutil.AV_PIX_FMT_BGR24),     // 24 bits per pixel, in BGR order
   BGRA8(1, 4, opencv_imgproc.COLOR_BGRA2RGBA, opencv_imgproc.COLOR_RGBA2BGRA, avutil.AV_PIX_FMT_BGRA),   // 32 bits per pixel, in BGRA order
   RGB8(1, 3, opencv_imgproc.COLOR_RGB2RGBA, opencv_imgproc.COLOR_RGBA2RGB, avutil.AV_PIX_FMT_RGB24),     // 24 bits per pixel, in RGB order
   RGBA8(1, 4, -1, -1, avutil.AV_PIX_FMT_RGBA),                             // 32 bits per pixel, in RGBA order
   YUV_I420(1, 1, opencv_imgproc.COLOR_YUV2RGBA_I420, opencv_imgproc.COLOR_RGBA2YUV_I420, -1),     // YUV420 format
   YUV_444P16(2, 3, -1, -1, avutil.AV_PIX_FMT_YUV444P16),                   // 16 bit planar YUV444, 48 bits per pixel.
   GRAY8(1, 1, opencv_imgproc.COLOR_GRAY2RGBA, opencv_imgproc.COLOR_RGBA2GRAY, avutil.AV_PIX_FMT_GRAY8),  // monochrome
   GRAY16(2, 1, -1, -1, avutil.AV_PIX_FMT_GRAY16),                          // aka depth
   UNKNOWN(-1, -1, -1, -1, -1);
   ...
   public int toOpenCVType(int numberOfChannels)
   {
      return switch ((int) bytesPerElement)
      {
         ...
         default -> throw new RuntimeException("Tomasz has been too busy (or lazy) to implement this. Feel free to do it yourself!");
      };
   }
