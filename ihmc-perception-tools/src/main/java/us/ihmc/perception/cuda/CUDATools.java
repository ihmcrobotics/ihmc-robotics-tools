...
public class CUDATools
{
   ...
   public static int getCUDADeviceCount()
   {
      ...
      checkCUDAError(cudart.cudaGetDeviceCount(devices));
      return devices[0];
   }
   ...
   {
      checkCUDAError(cudart.cudaMallocAsync(cudaPointer, cudaPointer.sizeof() * elementCount, stream));
   }
   ...
   {
      checkCUDAError(cudart.cudaMemcpyAsync(destination, source, source.sizeof() * elementCount, cudart.cudaMemcpyDefault, stream));
   }
   ...
   {
      if (errorCode != cudart.CUDA_SUCCESS)
      {
         try (BytePointer errorName = cudart.cudaGetErrorName(errorCode);
              BytePointer errorString = cudart.cudaGetErrorString(errorCode))
         {
   ...
   {
      if (errorCode != nvjpeg.NVJPEG_STATUS_SUCCESS)
      {
   ...
   {
      if (errorCode == nvrtc.NVRTC_SUCCESS)
         return;

      try (BytePointer errorString = nvrtc.nvrtcGetErrorString(errorCode))
      {
