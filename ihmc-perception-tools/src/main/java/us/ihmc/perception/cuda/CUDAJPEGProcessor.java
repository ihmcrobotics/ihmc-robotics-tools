...
public class CUDAJPEGProcessor
{
   ...
   public CUDAJPEGProcessor(int quality)
   {
      ...
      checkNVJPEGError(nvjpeg.nvjpegCreateSimple(nvjpegHandle));

      // Initialize nvjpeg state (used for decoding)
      nvjpegState = new nvjpegJpegState();
      checkNVJPEGError(nvjpeg.nvjpegJpegStateCreate(nvjpegHandle, nvjpegState));

      // Initialize encoder state
      encoderState = new nvjpegEncoderState();
      checkNVJPEGError(nvjpeg.nvjpegEncoderStateCreate(nvjpegHandle, encoderState, cudaStream));

      // Initialize encoder parameters
      encoderParameters = new nvjpegEncoderParams();
      checkNVJPEGError(nvjpeg.nvjpegEncoderParamsCreate(nvjpegHandle, encoderParameters, cudaStream));
      checkNVJPEGError(nvjpeg.nvjpegEncoderParamsSetQuality(encoderParameters, quality, cudaStream));
   }
   ...
   public void encodeYUV420(BytePointer sourceImage, BytePointer outputImagePointer, int imageWidth, int imageHeight)
   {
      ...
      checkNVJPEGError(nvjpeg.nvjpegEncoderParamsSetSamplingFactors(encoderParameters, nvjpeg.NVJPEG_CSS_420, cudaStream));

      // Get Y plane data
      BytePointer yPlanePointer = new BytePointer(); // create a pointer for the Y plane
      checkCUDAError(cudart.cudaMallocAsync(yPlanePointer, frameSize, cudaStream)); // allocate Y plane memory
      checkCUDAError(cudart.cudaMemcpyAsync(yPlanePointer, sourceImage, frameSize, cudart.cudaMemcpyHostToDevice, cudaStream)); // copy Y plane data to device memory
      ...
      checkCUDAError(cudart.cudaMallocAsync(uPlanePointer, quarterOfFrameSize, cudaStream));
      checkCUDAError(cudart.cudaMemcpyAsync(uPlanePointer, sourceImage, quarterOfFrameSize, cudart.cudaMemcpyHostToDevice, cudaStream));
      ...
      checkCUDAError(cudart.cudaMallocAsync(vPlanePointer, quarterOfFrameSize, cudaStream));
      checkCUDAError(cudart.cudaMemcpyAsync(vPlanePointer, sourceImage, quarterOfFrameSize, cudart.cudaMemcpyHostToDevice, cudaStream));
      nvjpegImage.pitch(2, halfOfImageWidth);
      nvjpegImage.channel(2, vPlanePointer);

      // Encode image (mem)
      checkNVJPEGError(nvjpeg.nvjpegEncodeYUV(nvjpegHandle, encoderState, encoderParameters, nvjpegImage, nvjpeg.NVJPEG_CSS_420, imageWidth, imageHeight, cudaStream));

      // Get compressed size
      SizeTPointer jpegSize = new SizeTPointer(1);
      checkNVJPEGError(nvjpeg.nvjpegEncodeRetrieveBitstream(nvjpegHandle, encoderState, (BytePointer) null, jpegSize, cudaStream));

      // Retrieve bitstream
      outputImagePointer.limit(jpegSize.get());
      checkNVJPEGError(nvjpeg.nvjpegEncodeRetrieveBitstream(nvjpegHandle, encoderState, outputImagePointer, jpegSize, cudaStream));

      // Free GPU memory
      checkCUDAError(cudart.cudaFreeAsync(yPlanePointer, cudaStream));
      checkCUDAError(cudart.cudaFreeAsync(uPlanePointer, cudaStream));
      checkCUDAError(cudart.cudaFreeAsync(vPlanePointer, cudaStream));
                                 ...
                                 BytePointer encodedImage)
   {
      ...
      // Set sampling factor
      checkNVJPEGError(nvjpeg.nvjpegEncoderParamsSetSamplingFactors(encoderParameters, nvjpeg.NVJPEG_CSS_444, cudaStream));

      // Upload image data to device
      BytePointer deviceImagePointer = new BytePointer();
      checkCUDAError(cudart.cudaMallocAsync(deviceImagePointer, totalSize, cudaStream));
      checkCUDAError(cudart.cudaMemcpy2DAsync(deviceImagePointer, rowSize, imageToEncode, imagePitch, rowSize, imageHeight, cudart.cudaMemcpyDefault, cudaStream));
      ...
      // Encode the image
      checkNVJPEGError(nvjpeg.nvjpegEncodeImage(nvjpegHandle,
                                                encoderState,
                                                encoderParameters,
                                                nvjpegImage,
      ...
      checkNVJPEGError(nvjpeg.nvjpegEncodeRetrieveBitstream(nvjpegHandle, encoderState, encodedImage, jpegSize, cudaStream));

      // Free GPU memory
      checkCUDAError(cudart.cudaFreeAsync(deviceImagePointer, cudaStream));
   ...
   public void decodeToBGR(BytePointer encodedImage, long encodedImageSize, Mat decodedImage)
   {
      ...
      long decodedImageSize = getOutputChannelSizes(nvjpeg.NVJPEG_OUTPUT_BGRI, imageInfo)[0];
      List<BytePointer> decodedData = new ArrayList<>();
      decodedData.add(new BytePointer());
      checkCUDAError(cudart.cudaMallocHost(decodedData.get(0), decodedImageSize));

      // Decode the image, packing result into the allocated buffer.
      decodeImage(encodedImage, encodedImageSize, imageInfo, nvjpeg.NVJPEG_OUTPUT_BGRI, decodedData);
      ...
      // Free all memory
      checkCUDAError(cudart.cudaFreeHost(decodedData.get(0)));
   ...
   public void decodeToBGR(BytePointer encodedImage, long encodedImageSize, GpuMat decodedImage)
   {
      ...
      List<BytePointer> decodedData = new ArrayList<>();
      decodedData.add(new BytePointer());
      checkCUDAError(cudart.cudaMallocAsync(decodedData.get(0), decodedImageSize, cudaStream));
      ...
      // Free all memory
      checkCUDAError(cudart.cudaFreeAsync(decodedData.get(0), cudaStream));
      decodedData.get(0).close();
      decodingResult.close();
   }
   ...
   public void decodeToYUV(BytePointer encodedImage, long encodedImageSize, Mat decodedImage)
   {
      ...
      int numberOfDecodedChannels = getNumberOfOutputChannels(nvjpeg.NVJPEG_OUTPUT_YUV, imageInfo);
      long[] decodedChannelSizes = getOutputChannelSizes(nvjpeg.NVJPEG_OUTPUT_YUV, imageInfo);
      ...
      for (int i = 0; i < numberOfDecodedChannels; ++i)
      {
         ...
         checkCUDAError(cudart.cudaMallocHost(decodedChannels.get(i), decodedChannelSizes[i]));
      }

      // Decode the image, packing result into the allocated buffers.
      decodeImage(encodedImage, encodedImageSize, imageInfo, nvjpeg.NVJPEG_OUTPUT_YUV, decodedChannels);
      ...
      for (int i = 0; i < numberOfDecodedChannels; ++i)
      {
         checkCUDAError(cudart.cudaFreeHost(decodedChannels.get(i)));
   ...
   public void decodeToGray(BytePointer encodedImage, long encodedImageSize, Mat decodedImage)
   {
      ...
      // Allocate host memory for the decoded image
      long decodedImageSize = getOutputChannelSizes(nvjpeg.NVJPEG_OUTPUT_Y, imageInfo)[0];
      List<BytePointer> decodedData = new ArrayList<>();
      decodedData.add(new BytePointer());
      checkCUDAError(cudart.cudaMallocHost(decodedData.get(0), decodedImageSize));

      // Decode only Y plane (luminosity)
      decodeImage(encodedImage, encodedImageSize, imageInfo, nvjpeg.NVJPEG_OUTPUT_Y, decodedData);
   ...
   public void decodeUnchanged(BytePointer encodedImage, long encodedImageSize, Mat decodedImage)
   {
      ...
      long[] decodedChannelSizes = getOutputChannelSizes(nvjpeg.NVJPEG_OUTPUT_UNCHANGED, imageInfo);
      List<BytePointer> decodedChannels = new ArrayList<>(numberOfDecodedChannels);
      for (int i = 0; i < numberOfDecodedChannels; ++i)
      {
         decodedChannels.add(new BytePointer());
      ...
      // Decode the image, packing result into the allocated buffers.
      decodeImage(encodedImage, encodedImageSize, imageInfo, nvjpeg.NVJPEG_OUTPUT_UNCHANGED, decodedChannels);
   ...
   public void decodeUnchanged(BytePointer encodedImage, long encodedImageSize, GpuMat decodedImage)
   {
      ...
      {
         decodedChannels.add(new BytePointer());
         checkCUDAError(cudart.cudaMallocAsync(decodedChannels.get(i), decodedChannelSizes[i], cudaStream));
      }
      ...
      for (int i = 0; i < numberOfDecodedChannels; ++i)
      {
         checkCUDAError(cudart.cudaFreeAsync(decodedChannels.get(i), cudaStream));
         decodedChannels.get(i).close();
      }
   }
                           ...
                           List<BytePointer> decodedChannels)
   {
      ...
      for (int i = 0; i < channelsToDecode; ++i)
      {
         ...
         checkCUDAError(cudart.cudaMallocAsync(decodedDeviceChannels[i], decodedChannelSizes[i], cudaStream));
      ...
      // Decode the image
      checkNVJPEGError(nvjpeg.nvjpegDecode(nvjpegHandle, nvjpegState, encodedImage, encodedImageSize, nvjpegOutputType, nvjpegDecodedImage, cudaStream));

      // Copy data to output pointers
      for (int i = 0; i < channelsToDecode; ++i)
      {
         checkCUDAError(cudart.cudaMemcpyAsync(decodedChannels.get(i), decodedDeviceChannels[i], decodedChannelSizes[i], cudart.cudaMemcpyDefault, cudaStream));
      }
      checkCUDAError(cudart.cudaStreamSynchronize(cudaStream));

      // Free all memory
      for (int i = 0; i < channelsToDecode; ++i)
      {
         checkCUDAError(cudart.cudaFreeAsync(decodedDeviceChannels[i], cudaStream));
   ...
   private int getNumberOfOutputChannels(int nvjpegOutputType, NVJPEGImageInfo imageInfo)
   {
      ...
      {
         case nvjpeg.NVJPEG_OUTPUT_Y, nvjpeg.NVJPEG_OUTPUT_RGBI, nvjpeg.NVJPEG_OUTPUT_BGRI -> channelsToDecode = 1;
         case nvjpeg.NVJPEG_OUTPUT_YUV, nvjpeg.NVJPEG_OUTPUT_RGB, nvjpeg.NVJPEG_OUTPUT_BGR -> channelsToDecode = 3;
         case nvjpeg.NVJPEG_OUTPUT_UNCHANGED -> channelsToDecode = imageInfo.numberOfComponents();
         case nvjpeg.NVJPEG_OUTPUT_UNCHANGEDI_U16 ->
   ...
   private long[] getOutputChannelSizes(int nvjpegOutputType, NVJPEGImageInfo imageInfo)
   {
      ...
      switch (nvjpegOutputType)
      {
         case nvjpeg.NVJPEG_OUTPUT_Y -> decodedChannelSizes[0] = imageInfo.width(0) * imageInfo.height(0);
         case nvjpeg.NVJPEG_OUTPUT_YUV, nvjpeg.NVJPEG_OUTPUT_UNCHANGED ->
         {
            for (int i = 0; i < channelsToDecode; ++i)
               decodedChannelSizes[i] = imageInfo.width(i) * imageInfo.height(i);
         }
         case nvjpeg.NVJPEG_OUTPUT_RGB, nvjpeg.NVJPEG_OUTPUT_BGR ->
         {
            for (int i = 0; i < channelsToDecode; ++i)
               decodedChannelSizes[i] = imageInfo.width(0) * imageInfo.height(0);
         }
         case nvjpeg.NVJPEG_OUTPUT_RGBI, nvjpeg.NVJPEG_OUTPUT_BGRI -> decodedChannelSizes[0] = 3L * imageInfo.width(0) * imageInfo.height(0);
         case nvjpeg.NVJPEG_OUTPUT_UNCHANGEDI_U16 ->
               throw new UnsupportedOperationException("NVJPEG_OUTPUT_UNCHANGEDI_U16 is currently unsupported. Please feel free to implement it!");
         default -> throw new IllegalArgumentException("The provided nvjpegOutputType is unknown.");
      }
   ...
   private long[] getOutputChannelPitches(int nvjpegOutputType, NVJPEGImageInfo imageInfo)
   {
      ...
      switch (nvjpegOutputType)
      {
         case nvjpeg.NVJPEG_OUTPUT_Y -> channelPitches[0] = imageInfo.width(0);
         case nvjpeg.NVJPEG_OUTPUT_YUV, nvjpeg.NVJPEG_OUTPUT_UNCHANGED ->
         {
            for (int i = 0; i < channelsToDecode; ++i)
         ...
         case nvjpeg.NVJPEG_OUTPUT_RGBI, nvjpeg.NVJPEG_OUTPUT_BGRI -> channelPitches[0] = 3L * imageInfo.width(0);
         case nvjpeg.NVJPEG_OUTPUT_UNCHANGEDI_U16 ->
               throw new UnsupportedOperationException("NVJPEG_OUTPUT_UNCHANGEDI_U16 is currently unsupported. Please feel free to implement it!");
         default -> throw new IllegalArgumentException("The provided nvjpegOutputType is unknown.");
      }
   ...
   private NVJPEGImageInfo getImageInfo(BytePointer encodedImage, long encodedImageSize)
   {
           ...
           IntPointer heightsPointer = new IntPointer())
      {
         ...
         checkCUDAError(cudart.cudaMallocHost(numberOfComponents, 1));
         checkCUDAError(cudart.cudaMallocHost(subSamplingsPointer, nvjpeg.NVJPEG_MAX_COMPONENT));
         checkCUDAError(cudart.cudaMallocHost(widthsPointer, nvjpeg.NVJPEG_MAX_COMPONENT));
         checkCUDAError(cudart.cudaMallocHost(heightsPointer, nvjpeg.NVJPEG_MAX_COMPONENT));

         // Extract the data
         checkNVJPEGError(nvjpeg.nvjpegGetImageInfo(nvjpegHandle,
                                                    encodedImage,
                                                    encodedImageSize,
                                                    numberOfComponents,
                                                    subSamplingsPointer,
         ...
         checkCUDAError(cudart.cudaFreeHost(numberOfComponents));
         checkCUDAError(cudart.cudaFreeHost(subSamplingsPointer));
         checkCUDAError(cudart.cudaFreeHost(widthsPointer));
         checkCUDAError(cudart.cudaFreeHost(heightsPointer));
   ...
   public void destroy()
   {
      checkNVJPEGError(nvjpeg.nvjpegEncoderParamsDestroy(encoderParameters));
      checkNVJPEGError(nvjpeg.nvjpegEncoderStateDestroy(encoderState));
      checkNVJPEGError(nvjpeg.nvjpegJpegStateDestroy(nvjpegState));
      checkNVJPEGError(nvjpeg.nvjpegDestroy(nvjpegHandle));
