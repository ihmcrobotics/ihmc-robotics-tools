...
public class CUDAProgram implements AutoCloseable
{
   ...
   private void initialize(String programName, String programCode, String[] headerNames, String[] headerContents, String[] compilationOptions)
   {
      ...
      error = nvrtc.nvrtcGetPTXSize(compiledProgram, ptxSize);
      checkNVRTCError(error);

      // Get the PTX
      BytePointer ptx = new BytePointer(ptxSize.get());
      error = nvrtc.nvrtcGetPTX(compiledProgram, ptx);
      checkNVRTCError(error);

      LogTools.debug("The following PTX was generated for {}:\n{}", programName, ptx.getString());

      // Load the module using the PTX
      error = cudart.cuModuleLoadData(module, ptx);
      checkCUDAError(error);

      // Release stuff
      nvrtc.nvrtcDestroyProgram(compiledProgram);
                                      ...
                                      _nvrtcProgram compiledProgram)
   {
      ...
      {
         // Create the program
         error = nvrtc.nvrtcCreateProgram(compiledProgram,
                                          programCodePointer,
                                          programNamePointer,
         ...
         // Compile the program
         error = nvrtc.nvrtcCompileProgram(compiledProgram, compilationOptions == null ? 0 : compilationOptions.length, options);
         checkNVRTCError(error);

         // In case of error, print compilation log
         if (error != nvrtc.NVRTC_SUCCESS)
   ...
   private static void printProgramLog(_nvrtcProgram program, String programName, Level logLevel)
   {
      ...
      error = nvrtc.nvrtcGetProgramLogSize(program, logSize);
      checkNVRTCError(error);
      // Get the log & print
      BytePointer log = new BytePointer(logSize.get());
      error = nvrtc.nvrtcGetProgramLog(program, log);
      checkNVRTCError(error);
   ...
   private static String getComputeVersion()
   {
      ...
      cudart.cudaGetDeviceProperties(deviceProperties, 0);
